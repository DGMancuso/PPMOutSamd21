...
void IRAM_ATTR PPMOut::onPpmTimer() {     
    ...
    if (ppmState == PPM_STATE_PULSE)
    {
        ...
        timerAlarmWrite(PpmTimer, PPM_PULSE_LENGTH, true);
    }
    else if (ppmState == PPM_STATE_FILL)
    {
        ...
        {
            ppmChannel = 0;
            timerAlarmWrite(PpmTimer, PPM_FRAME_LENGTH - usedFrameLength, true);
            usedFrameLength = 0;
        }
        else
        {
            usedFrameLength += currentChannelValue - PPM_PULSE_LENGTH;
            timerAlarmWrite(PpmTimer, currentChannelValue - PPM_PULSE_LENGTH, true);
        }
    }
    portEXIT_CRITICAL(&timerMux);
    digitalWrite(PPM_OUT_PIN, ppmOutput);
}
